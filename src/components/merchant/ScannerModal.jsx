...
export default function ScannerModal({ isOpen, onClose, businessId }) {
    ...
    const onScanSuccess = async (decodedText, decodedResult) => {
        // Expected format: tagdeer://vip/USER_UUID
        if (scannerRef.current) {
        ...
        try {
            if (!decodedText.startsWith('tagdeer://vip/')) {
                throw new Error('Invalid Tagdeer QR Code format.');
            }

            const targetUserId = decodedText.split('tagdeer://vip/')[1];
            ...
            // Mock Supabase error bypass for UI testing
    ...
    const handleManualSubmit = (e) => {
        ...
        setScanStatus('processing');
        // Simulate network call for manual OTP
        setTimeout(() => {
            setScannedUserId("mock-user-id");
            setScanStatus('success');
            toast.success('Interaction logged successfully via code!');
        }, 1000);
    };
    ...
    const handleAllocateCoupon = (couponName) => {
        toast.success(`Coupon "${couponName}" has been pushed to the user's wallet!`);
        onClose();
    };

    // Mock active coupons for physically pushing
    const mockCoupons = [
        { id: '1', name: '10% Off Coffee', remaining: 45 },
        { id: '2', name: 'Free Dessert', remaining: 2 }
    ];
                                ...
                                {!showCoupons ? (
                                    ...
                                    <div className="w-full border rounded-xl overflow-hidden animate-in slide-in-from-bottom-2 duration-300 bg-slate-50/50">
                                        ...
                                        {mockCoupons.map(coupon => (
                                            <button
                                                key={coupon.id}
                                                onClick={() => handleAllocateCoupon(coupon.name)}
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="font-semibold text-slate-800">{coupon.name}</span>
                                                </div>
